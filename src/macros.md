...
* [`宏规则（macro_rules）`][`macro_rules`]转换
* [外部块][External blocks]

当用作项或语句时，_MacroInvocationSemi_ 形式被使用。_MacroInvocationSemi_ 形式中，如果不使用大括号，则在结尾处需要分号。在宏调用或[`宏规则（macro_rules）`][`macro_rules`]定义之前，不允许使用[可见性限定符][Visibility qualifiers]。 
